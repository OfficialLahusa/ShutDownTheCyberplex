...
/**
 * Partikelsystem, das beim Treffen von Drohnen mit Schüssen benutzt wird
 * 
 * @author Lasse Huber-Saffer
 * @version 01.01.2022
 */
public class DroneHitParticleSystem implements IParticleSystem
{
    // Parent
    private Room _room;
     ...
     */
    public DroneHitParticleSystem(Vector3 position, Room room, HashMap<String, Mesh> particleMeshes)
    {
        if(position == null)
        {
            throw new IllegalArgumentException("position was null when constructing DroneHitParticleSystem");
        }
        if(room == null)
        {
            throw new IllegalArgumentException("room was null when constructing DroneHitParticleSystem");
        }
        if(particleMeshes == null)
        {
            throw new IllegalArgumentException("particleMeshes was null when constructing DroneHitParticleSystem");
        }
        ...
        _colors = new TurtleColor[] { TurtleColor.RED, TurtleColor.YELLOW, TurtleColor.ORANGE };
        
        for(int i = 0; i < HIT_PARTICLE_COUNT; i++)
        {
            Vector3 randomDirection = new Vector3(_random.nextDouble() * 2.0 - 1.0, _random.nextDouble() * 2.0, _random.nextDouble() * 2.0 - 1.0).normalize();
            ...
            _particles.add(
                new BillboardParticle(
                    ...
                    randomDirection.multiply(_random.nextDouble() * 6.0 + 6.0), new Vector3(0.0, -9.81 * 2.0, 0.0),
                    null
                )
            );
        }
